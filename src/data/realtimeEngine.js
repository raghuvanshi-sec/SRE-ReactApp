...
const agentLogTemplates = {
  "agent-1": [
    (c) =>
      `${timestamp()} - Evaluated ${c.name} Churn Risk (${c.churnRisk}%). Running retention simulations...`,
    (c) =>
      `${timestamp()} - Generated dynamic discount play for ${c.name}. Awaiting governance approval.`,
    () =>
      `${timestamp()} - Auto-applied loyalty extension to healthy accounts. 3 accounts retained.`,
    (c) =>
      `${timestamp()} - Detected sentiment drop for ${c.name}. Triggering proactive outreach sequence.`,
    () =>
      `${timestamp()} - Completed batch risk re-scoring. 2 accounts upgraded to low-risk status.`,
    (c) =>
      `${timestamp()} - Initiated escalation workflow for ${c.name}. CS team notified via Slack.`,
  ],
  "agent-2": [
    (c) =>
      `${timestamp()} - Sent onboarding follow-up to ${c.name}. Tracking engagement metrics.`,
    (c) =>
      `${timestamp()} - Negotiated plan migration for ${c.name}. Contract preserved at lower tier.`,
    () =>
      `${timestamp()} - Processed 5 mid-market renewal reminders. 4 auto-renewed successfully.`,
    (c) =>
      `${timestamp()} - Detected usage anomaly for ${c.name}. Active users declined by 40%.`,
    () =>
      `${timestamp()} - Launched interactive chatbot session for 3 accounts with low engagement.`,
  ],
  "agent-3": [
    (c) =>
      `${timestamp()} - Initiated incident recovery workflow for ${c.name}. SLA review in progress.`,
    () =>
      `${timestamp()} - Calculated SLA penalty credits for Q2 incidents. Total: $12,400 across 3 accounts.`,
    (c) =>
      `${timestamp()} - Routing ${c.name} to priority support queue. Estimated wait: 0 minutes.`,
    () =>
      `${timestamp()} - Service health check complete. All production endpoints nominal.`,
    (c) =>
      `${timestamp()} - Generated compensation package for ${c.name}. Pending admin sign-off.`,
  ],
};
  ...
  return kpis.map((kpi) => {
    ...
    switch (kpi.id) {
      case "kpi-1": {
        ...
        const change =
          delta < 0 ? `${delta.toFixed(1)}% MoM` : `+${delta.toFixed(1)}% MoM`;
      ...
      case "kpi-5": {
        // Pending Approvals — updated by approval engine
        break;
      }
    }
    return next;
...
function evolveCustomers(customers) {
  return customers.map((cust) => {
    ...
    next.digitalTwin = {
      ...cust.digitalTwin,
      retentionProbability: { ...cust.digitalTwin.retentionProbability },
    };
...
function evolveAgents(agents, customers) {
  return agents.map((agent) => {
    ...
    if (agent.status === "active") {
      ...
      next.accuracy = Math.min(
        99.9,
        Math.max(80, agent.accuracy + randFloat(-0.3, 0.2)),
      );

      // Add a new log entry (pick a random customer for context)
      const templates =
        agentLogTemplates[agent.id] || agentLogTemplates["agent-1"];
...
function maybeAddInteraction(customers) {
  ...
  return customers.map((cust, i) => {
    ...
    return {
      ...cust,
      recentInteractions: [newInteraction, ...cust.recentInteractions].slice(
        0,
        6,
      ),
    };
...
export function useRealtimeData(tickIntervalMs = 5000) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const activeCount = nextAgents.filter(
        (a) => a.status === "active",
      ).length;
      nextKPIs = nextKPIs.map((kpi) =>
        kpi.id === "kpi-4"
          ? {
              ...kpi,
              value: `${activeCount} / ${nextAgents.length}`,
              change: `${Math.round((activeCount / nextAgents.length) * 100)}% operational`,
            }
          : kpi,
      );
      // Sync pending approvals count
      const pendingCount = approvalsRef.current.filter(
        (a) => a.status === "pending",
      ).length;
      nextKPIs = nextKPIs.map((kpi) =>
        kpi.id === "kpi-5"
